  ...
  try {
    ...
    */
    ...
    /* If something goes wrong while communicating with the database, log the error and send
       an error message back to the client. Note that our return will return us from the setCName
...
const setDName = async (req, res) => {
  /* If we look at views/page2.handlebars, the form has inputs for a name,
     and an age. When this POST request is sent to us, the bodyParser plugin
     we configured in app.js will store that information in req.body for us.
  */
  ...
  */
  try {
    /* newDog is a version of our dogData that is database-friendly. If you print it, you will
      ...
      the database. Since we have never saved this dog before, .save() will create a new dpg in
      the database. All calls to the database are async, including .save() so we will await the
      databases response. If something goes wrong, we will end up in our catch() statement. If
    ...
    */
    ...
    /* If something goes wrong while communicating with the database, log the error and send
      an error message back to the client. Note that our return will return us from the setDName
      function, not just the catch statement. That means we can treat the code below the catch
      as being our "if the try worked"
    */
    console.log(err);
...
const searchName = async (req, res) => {
  /* When the user makes a POST request, bodyParser populates req.body with the parameters
    as we saw in setCName() above. In the case of searchName, the user is making a GET request.
  ...
  */
  let doc;
  try {
    /* Just like Dog.find() in hostPage1() above, Mongoose models also have a .findOne()
...
// Function to handle searching a dog by name. Will use post
const ageByName = (req, res) => {
  /* When the user makes a POST request, bodyParser populates req.body with the parameters
